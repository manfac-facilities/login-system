...
- consultar a OS antiga direto no Field (`GET /orders/:id` do `field_id` antigo);
- **antiga arquivada ou inexistente** → a obra existente recebe o `field_id` novo, limpa
  suspeita e alerta, e o relatório mostra **"OS reaberta: histórico herdado"**;
