...
| `obras-sync-execucao` | **parcial em 2026-09-14** (frente D3 do Duda): tabela `obras_sync_execucao`, índices (incl. trava `obras_sync_execucao_uma_rodando`), RLS só admin e a função `obras_iniciar_sync_execucao`. **Os jobs `obras-field-incremental` e `obras-field-completa` NÃO existem** — criar só depois de o cliente liberar a primeira carga, com `OBRAS_CRON_SECRET` no Vault e a completa em `5 6 * * *`. ⚠️ O banco já tem 2 jobs que não são deste repo: `field-sync-every-5-min` (chama a Edge Function `sync`) e `financeiro-producao-catalogo` — não mexer sem saber de quem são |
