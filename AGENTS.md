...
| CRM | `/crm` | **CRM** | **Faltava nesta tabela até 20/09/2026** — existe desde a Frente B do site e ninguém que lesse só este arquivo saberia. Lista os leads capturados em `manfac.com.br` (tabela `site_leads`). Slug de acesso: `crm`, em `lib/sistemas.ts:12`; rota no `matcher` (`middleware.ts:131`). ⚠️ `site_leads` **não tem policy de RLS nenhuma** e a página usa `createAdminClient()`: a checagem de acesso é repetida dentro de `app/crm/page.tsx` de propósito, porque a do middleware é "optimistic" segundo a doc deste Next. Não remova essa segunda checagem |
...
| `obras-field-reconciliacao` | **aplicado em 2026-09-14**, pela Management API (frente D2 do Duda). Colunas `field_id`, `field_ausente_desde` e `field_ausente_em`, nullable; índice `obras_obra_field_id_unico` (parcial, `field_id is not null`); checks `obras_obra_field_id_nao_vazio` e `obras_obra_alerta_field_tem_suspeita` |
| `obras-sync-execucao` | **parcial em 2026-09-14** (frente D3 do Duda): tabela `obras_sync_execucao`, índices (incl. trava `obras_sync_execucao_uma_rodando`), RLS só admin e a função `obras_iniciar_sync_execucao`. **Os jobs `obras-field-incremental` (`*/5 * * * *`) e `obras-field-completa` (`5 6 * * *`) foram criados em 2026-09-16**, junto com o segredo `OBRAS_CRON_SECRET` no Vault (`sdd-sql-obras-cron-jobs.sql`), depois da primeira carga real — criar só depois de o cliente liberar a primeira carga, com `OBRAS_CRON_SECRET` no Vault. **A completa foi movida para `2 6 * * *` em 2026-09-20**: em `5 6` ela colidia com a incremental (`*/5`), perdia a trava e morria em silêncio — não rodou nenhuma vez entre 17 e 20/09. ⚠️ O banco tem 3 jobs que não são deste repo: `field-sync-every-5-min` (chama a Edge Function `sync`), `financeiro-producao-catalogo` e `financeiro-producao-lancamento` — não mexer sem saber de quem são |
| `obras-historico` | **aplicado em 2026-09-20**, pela Management API. Tabela `obras_historico` com RLS e a RPC `obras_aplicar_alteracao` |
| `obras-motivos-remarcacao` | **aplicado em 2026-09-20**, pela Management API. `obras_motivo_remarcacao` com os 6 motivos de fábrica, `detalhe`/`registrado_por` em `obras_remarcacao` e a RPC `obras_remarcar_inicio`. A verificação do fim do arquivo devolveu **18 linhas, todas `OK`**. ⚠️ O código que usa essas duas **ainda não foi deployado** em 20/09 |
